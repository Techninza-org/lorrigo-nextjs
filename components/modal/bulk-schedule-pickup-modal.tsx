...
export const BulkSchedulePickupModal = () => {
    const { isOpen, onClose, type, data } = useModal();
    const router = useRouter();

    const { b2bOrder, orders } = data;

    const { bulkManifestOrder } = useSellerProvider();

    const isModalOpen = isOpen && type === "bulkPickupSchedule";
    ...
    const onSubmit = async (values: z.infer<typeof schema>) => {
        try {
            const res = await bulkManifestOrder({ orderIds: orders?.map(x => x._id) || [], scheduleDate: formatDate(`${values.schedulePickup}`, 'dd-MM-yyyy') })
            if (res) {
                form.reset();
                router.refresh();
                onClose();
            ...
            <DialogContent className="bg-white dark:text-white text-black p-0 overflow-hidden">
                <DialogHeader className="pt-8 px-6">
                    ...
                    {/* <DialogDescription className='pt-3'>
                        Schedule Pickup for Order: <span className='font-medium underline underline-offset-4 text-blue-800'>{order?.order_reference_id || b2bOrder?.order_reference_id}</span>
                    </DialogDescription> */}
                </DialogHeader>
                <Form {...form}>
