      ...
      <CardContent>
        <Alert className={cn(verified ? 'bg-green-300' : 'bg-amber-300')}>
          ...
          <AlertTitle className='text-lg'>
            {verified ? "Congratulations" : "Pending"}
          </AlertTitle>
          <AlertDescription>
        ...
        <div className='p-3 flex flex-col md:flex-row gap-6'>
          <Image
            src={`data:image/jpeg;base64,${seller?.kycDetails?.photoUrl || formData?.photoUrl || ""}`}
            alt="photo"
            width={330}
            height={330}
          ...
          <div className='w-full'>
            <div className='flex flex-col md:flex-row justify-between w-full px-3 py-4'>
              <p className='space-x-2'>
                <span className='text-gray-500'>KYC Status:</span> 
                <span className={cn(verified ? "text-green-500" : "text-yellow-500")}>
                  {verified ? "Verified" : "Pending"}
                </span>
              </p>
              <p className='space-x-2'>
                <span className='text-gray-500'>Current business type:</span> 
                <span className='capitalize'>
                  {seller?.kycDetails.businessType || formData?.businessType}
                </span>
              </p>
            </div>
              ...
              <div>
                <div className='grid grid-cols-2'>
                  ...
                  <div className='capitalize'>
                    {seller?.kycDetails.document1Type || formData?.document1Type}
                  </div>
                </div>
                <div className='grid grid-cols-2'>
                  <div>Document 1 No:</div>
                  <div className='capitalize'>
                    {seller?.kycDetails.document1Feild || formData?.document1Feild || 1}
                  </div>
                </div>
              </div>
              <div>
                <div className='grid grid-cols-2'>
                  ...
                  <div className='capitalize'>
                    {seller?.kycDetails.document2Type || formData?.document2Type}
                  </div>
                </div>
                <div className='grid grid-cols-2'>
                  <div>Document 2 No:</div>
                  <div className='capitalize'>
                    {seller?.kycDetails.document2Feild || formData?.document2Feild || 2}
                  </div>
                </div>
              </div>
            </div>
